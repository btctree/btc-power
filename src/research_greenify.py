"""Can we make Growth A / Aggressive B green EVERY year? Progressively add the 'safety stack'
(short-selective shorts, flat-in-chop, stronger dd-kill, lower cap) and watch the losing years
(2018/2022/2025) AND the total return. Same verified engine as build_full_compare. 50bp.
"""
...
def lmap(p, c):
    return dict(zip(pd.read_csv(p).iloc[:, 0].astype(str), pd.read_csv(p)[c])) if os.path.exists(p) else {}
...
def make_sig(short_sel, chop_flat):
    s = esm.copy()
    for i in range(n):
        if short_sel and s[i] < 0 and not (reg[i] in ("STRONG_DOWN", "TREND_DOWN") and close[i] < sma200[i]): s[i] = 0.0
        if chop_flat and reg[i] == "CHOP_HIVOL": s[i] = 0.0
    return s

def sim(sig, cap, vt, band, slip, dd_kill=0.30, fee=0.0005, maint=0.01):
...
# base Growth A (cap5,vt1.5,band.15) and Aggressive B (cap5,vt2,band.25), then progressive safety stack
tests = [
    ("Growth A (base)", make_sig(0, 0), 5, 1.5, 0.15, 0.30),
    ("Growth A +short-sel", make_sig(1, 0), 5, 1.5, 0.15, 0.30),
    ("Growth A +short-sel+chopflat", make_sig(1, 1), 5, 1.5, 0.15, 0.30),
    ("Growth A +all+ddkill.20", make_sig(1, 1), 5, 1.5, 0.15, 0.20),
    ("Growth A +all+cap3", make_sig(1, 1), 3, 1.5, 0.15, 0.20),
    ("Aggressive B (base)", make_sig(0, 0), 5, 2.0, 0.25, 0.30),
    ("Aggressive B +full safety", make_sig(1, 1), 3, 2.0, 0.25, 0.20),
]
print(f"{'variant':32s} " + "".join(f"{y:>6}" for y in YEARS) + "  |  FINAL   neg-yrs")
for nm, sig, cap, vt, band, ddk in tests:
    eq = sim(sig, cap, vt, band, 0.005, ddk); Y = yr(eq)
    neg = sum(1 for y in YEARS if Y[y] == Y[y] and Y[y] < 0)
    row = "".join((f"{Y[y]*100:>+5.0f}%" if Y[y] == Y[y] else f"{'—':>6}") for y in YEARS)
    print(f"{nm:32s} {row}  | ${eq[-1]:>11,.0f}  {neg}")
