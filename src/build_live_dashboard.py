...
$('b8_actions').innerHTML=PA.length?PA.slice().reverse().map(a=>{const dp=(a.delta_pct!=null)?((a.delta_pct>0?'+':'')+a.delta_pct+'%'):'';const dx=a.type!=='ENTER'?(' '+(a.delta_x>0?'+':'')+a.delta_x+'× ('+dp+')'):(' '+a.to_x+'× ('+dp+')');
